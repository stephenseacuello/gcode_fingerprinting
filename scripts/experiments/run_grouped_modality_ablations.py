#!/usr/bin/env python3
"""
Run Grouped Modality Ablation Studies for G-code Fingerprinting.

This script runs ablation studies for GROUPED modalities:
...
Modality Groups (9 total):
  Sensor Groups (6):
    - Accelerometer: Ax, Ay, Az (36 features from 12 sensors)
    - Gyroscope: Gx, Gy, Gz (36 features)
    - Magnetometer: Mx, My, Mz (36 features)
    - Environmental: Pressure, Temperature, Proximity (36 features)
    - Color: ColorR, ColorG, ColorB, ColorA (48 features)
    - RMS: RMS (12 features)
...
Usage:
    python scripts/experiments/run_grouped_modality_ablations.py \
        --output-dir outputs/grouped_modality_ablations \
...
# Sensor modality groups (sensor_id.modality format)
...
# Machine feature column name patterns
...
# All modality groups combined
ALL_GROUPS = list(SENSOR_MODALITY_GROUPS.keys()) + list(MACHINE_FEATURE_PATTERNS.keys())


def get_group_mask(
    master_columns: List[str],
    groups_to_ablate: Optional[List[str]] = None,
    groups_to_keep: Optional[List[str]] = None,
) -> np.ndarray:
    """
    Create a mask for grouped modality ablation.

    Args:
        master_columns: List of all column names
        groups_to_ablate: If set, mask these groups (leave-one-out)
        groups_to_keep: If set, only keep these groups (leave-one-in)
    ...
    for i, col in enumerate(master_columns):
        ...
        if len(parts) >= 2:
            modality = parts[1]
            for group_name, modalities in SENSOR_MODALITY_GROUPS.items():
        ...
        if matched_group:
            if groups_to_ablate is not None:
                if matched_group in groups_to_ablate:
                    mask[i] = 0.0
            elif groups_to_keep is not None:
                if matched_group not in groups_to_keep:
                    mask[i] = 0.0
...
def run_grouped_ablations(
    ...
    if ablation_type in ['both', 'leave_one_out']:
        print("\n" + "=" * 70)
        print("LEAVE-ONE-OUT: Removing each group")
        print("=" * 70)
        for group_name in ALL_GROUPS:
            mask = get_group_mask(master_columns, groups_to_ablate=[group_name])
    ...
    # Leave-one-in for each group
    if ablation_type in ['both', 'leave_one_in']:
        print("\n" + "=" * 70)
        print("LEAVE-ONE-IN: Using only each group")
        print("=" * 70)
        for group_name in ALL_GROUPS:
            mask = get_group_mask(master_columns, groups_to_keep=[group_name])
...
def generate_report(all_results: Dict, output_dir: str):
    ...
    report = {
        'metadata': {
            ...
            'sensor_modality_groups': SENSOR_MODALITY_GROUPS,
    ...
    # Save report
    with open(output_path / 'grouped_modality_ablation_report.json', 'w') as f:
        json.dump(report, f, indent=2, default=str)

    # Print summary
    print("\n" + "=" * 80)
    print("GROUPED MODALITY ABLATION SUMMARY")
    print("=" * 80)
    ...
    if group_importance_loo:
        ...
        for group_name, importance in sorted_loo:
            ...
            print(f"  {group_name:20s}: {sign}{abs(importance)*100:.2f}%")

    if group_importance_loi:
        print("\n--- LEAVE-ONE-IN (accuracy with only this group) ---")
        sorted_loi = sorted(group_importance_loi.items(), key=lambda x: -x[1])
        for group_name, acc in sorted_loi:
            print(f"  {group_name:20s}: {acc*100:.2f}%")

    print(f"\nFull report saved to: {output_path / 'grouped_modality_ablation_report.json'}")


def main():
    parser = argparse.ArgumentParser(description='Run grouped modality ablation studies')
    ...
    # Run ablations
    all_results = run_grouped_ablations(
        config=config,
    ...
    print(f"\n{'='*60}")
    print("GROUPED MODALITY ABLATION STUDY COMPLETE")
    print(f"{'='*60}")
