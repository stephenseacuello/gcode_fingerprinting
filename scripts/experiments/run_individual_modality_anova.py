...
def get_single_modality_mask(
    master_columns: List[str],
    modality: str,
    include_machine_features: bool = True,
) -> np.ndarray:
    """Create mask for a single modality across all operational sensors."""
    mask = np.zeros(len(master_columns), dtype=np.float32)

    for i, col in enumerate(master_columns):
        # Check if this column is the target modality for any operational sensor
        for sensor in OPERATIONAL_SENSORS:
            if col == f'{sensor}.{modality}':
                mask[i] = 1.0
...
def get_machine_group_mask(master_columns: List[str], group_name: str) -> np.ndarray:
    """Create mask for a machine feature group only."""
    mask = np.zeros(len(master_columns), dtype=np.float32)
    features = MACHINE_GROUPS[group_name]
...
def run_anova(results_by_group: Dict[str, List[float]], output_dir: Path):
    ...
    print("\n" + "=" * 70)
    print("ONE-WAY ANOVA: INDIVIDUAL MODALITY COMPARISON")
    print("=" * 70)
    ...
    print(f"\nGroup Statistics (sorted by mean accuracy):")
    print(f"  {'Group':<25} {'Mean':>10} {'Std':>10} {'N':>5}")
    print("-" * 55)
    ...
    for group in sorted_groups:
        ...
        print(f"  {group:<25} {np.mean(vals)*100:>9.2f}% {np.std(vals)*100:>9.2f}% {len(vals):>5}")
    ...
    if p_value < 0.05:
        ...
        try:
            ...
            if sig_pairs:
                ...
                for g1, g2, diff, pval in sig_pairs[:20]:
...
def main():
    parser = argparse.ArgumentParser(description='One-way ANOVA for individual modality comparison')
    ...
    # 17 individual sensor modalities
    for modality in INDIVIDUAL_MODALITIES:
        mask = get_single_modality_mask(
            master_columns, modality,
            include_machine_features=args.include_machine_features
        )
        configs[f'mod_{modality}'] = mask
        print(f"  {modality}: {mask.sum():.0f} features")

    # 3 machine feature groups
    for group_name in MACHINE_GROUPS.keys():
        mask = get_machine_group_mask(master_columns, group_name)
    ...
    with open(output_dir / 'all_results.json', 'w') as f:
        json.dump({
            'metadata': {
                ...
                'individual_modalities': INDIVIDUAL_MODALITIES,
    ...
    print(f"\n{'='*60}")
    print("INDIVIDUAL MODALITY ANOVA COMPLETE")
    print(f"{'='*60}")
