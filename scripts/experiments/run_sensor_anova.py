#!/usr/bin/env python3
"""
One-Way ANOVA for Sensor Ablation Studies.

Trains models for each of 12 sensors (+ all-sensors baseline) with multiple seeds,
then runs one-way ANOVA to test if sensor differences are statistically significant.

Groups (13):
  - 12 individual sensors (leave-one-in)
  - 1 baseline (all 16 sensors including non-functional)
...
# 4 non-functional sensors (included in baseline)
...
# Machine features (always included)
MACHINE_FEATURES = [
    # Motor electrical (8)
...
def get_single_sensor_mask(master_columns: List[str], sensor_id: str) -> np.ndarray:
    """Create mask that keeps only one sensor + machine features."""
    mask = np.zeros(len(master_columns), dtype=np.float32)

    for i, col in enumerate(master_columns):
        # Keep machine features
        if col in MACHINE_FEATURES:
            mask[i] = 1.0
            continue

        # Keep specified sensor
        if col.startswith(f'{sensor_id}.'):
            mask[i] = 1.0

    return mask


def get_all_sensors_mask(master_columns: List[str]) -> np.ndarray:
...
def run_anova(results_by_group: Dict[str, List[float]], output_dir: Path):
    ...
    print("\n" + "=" * 70)
    print("ONE-WAY ANOVA: SENSOR COMPARISON")
    print("=" * 70)
    ...
    if p_value < 0.05:
        ...
        try:
            ...
            if sig_pairs:
                ...
                for g1, g2, diff, pval in sig_pairs[:10]:  # Top 10
...
def main():
    parser = argparse.ArgumentParser(description='One-way ANOVA for sensor comparison')
    ...
    parser.add_argument('--include-baseline', action='store_true', default=True,
                        help='Include all-sensors baseline in ANOVA')
    ...
    for sensor_id in OPERATIONAL_SENSORS:
        mask = get_single_sensor_mask(master_columns, sensor_id)
        configs[f'sensor_{sensor_id}'] = mask

    # Baseline: all sensors
    if args.include_baseline:
        configs['all_sensors'] = get_all_sensors_mask(master_columns)
