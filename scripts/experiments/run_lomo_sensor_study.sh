#!/usr/bin/env bash
# =============================================================================
# Leave-one-SENSOR-out encoder study (full_window).  Companion to the
# modality-level LOMO -- this cuts along the orthogonal axis: it drops one
# entire PHYSICAL SENSOR UNIT (all 15 of its channels) at a time, retrains the
# encoder, and trains a fresh decoder. Answers the deployment-real question
# "which sensor board can the machine ship without?", which the modality
# study (drop a channel-type across all boards) does not.
#
# The 6 physical sensors each carry 15 channels (3 accel + 3 gyro + 3 magnet +
# 4 color + 1 temp + 1 rms); electrical (8 machine columns) is not a board.
#
# Per cell (sensor S, fold F): preprocess full_window with S excluded ->
# capture consumed columns -> channel-identity gate (--group-kind sensor) ->
# retrain encoder -> retrain decoder (headline recipe). Identical pipeline to
# run_lomo_encoder_study.sh; only the exclusion axis differs.
#
# Env: GPU, SENSORS, FOLDS, EPOCHS, PATIENCE, SUMMARY_TAG.
# =============================================================================
set -uo pipefail
cd "$(dirname "$0")/../.."
...
ROOT="${ROOT:-outputs/decoder20260511/lomo_sensor}"
GPU="${GPU:-0}"
SEED="${SEED:-42}"
N_FOLDS=5
EPOCHS="${EPOCHS:-300}"
...
# 'baseline' (no sensor removed) first; then each of the 6 physical sensors.
SENSORS="${SENSORS:-baseline frame_l2 frame_l3 frame_r2 spindle2 y_bed__3 y_bed__4}"
...
: > "$SUMMARY"
echo -e "sensor\tfold\tgate\tencoder_ckpt\tdecoder_command\tstatus" >> "$SUMMARY"

run_cell() {  # $1=sensor $2=fold
  local s="$1" f="$2"
  local cell="${ROOT}/lomo/${s}/fold_${f}"
  local FW="${cell}/full_window" ENC="${cell}/encoder" DEC="${cell}/decoder"
  mkdir -p "$FW" "$ENC" "$DEC"
  local is_base=false; [[ "$s" == "baseline" ]] && is_base=true
  ...
  if [ -n "$free" ] && [ "$free" -lt "$MIN_FREE_GB" ]; then
    ...
    echo -e "${s}\t${f}\t-\t-\t-\tdisk_abort" >> "$SUMMARY"; exit 3
  fi

  local excl=""
  $is_base || excl="--exclude-sensor ${s}"
  ...
  python3 scripts/preprocessing/run_preprocessing_v8_cv_fold.py \
    ...
    || { log "preprocess errored (see ${cell}/preprocess.log) -> skip"; \
         echo -e "${s}\t${f}\t-\t-\t-\tpreprocess_error" >> "$SUMMARY"; return 0; }
...
PY
  if [[ ! -s "${ENC}/consumed_columns.json" ]]; then
    ...
    echo -e "${s}\t${f}\t-\t-\t-\tcols_error" >> "$SUMMARY"; return 0
  fi

  # 3. channel-identity gate (sensor mode; baseline defines the reference) ---
  ...
  CUDA_VISIBLE_DEVICES="$GPU" python3 scripts/evaluation/run_decoder_quick_test.py \
    ...
    || { log "decoder errored (see ${cell}/decoder.log)"; \
         echo -e "${s}\t${f}\tPASS\t${ckpt}\t-\tdecoder_error" >> "$SUMMARY"; return 0; }
  ...
  echo -e "${s}\t${f}\t$( $is_base && echo NA || echo PASS )\t${ckpt}\t${cmd}\tok" >> "$SUMMARY"
}

log "LOMO sensor-level | sensors=[${SENSORS}] | folds=[${FOLDS}] | GPU=${GPU} | epochs=${EPOCHS}"
for s in $SENSORS; do for f in $FOLDS; do run_cell "$s" "$f"; done; done
