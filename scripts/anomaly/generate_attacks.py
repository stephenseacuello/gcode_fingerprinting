#!/usr/bin/env python3
"""Phase 0b: Pre-generate all attack token sequences.

Generates A1-A9 attack variants for all fold test sets and saves them
so that downstream experiments can load pre-built attacks without
re-computing them.

Also produces vocab_analysis.json with per-axis value inventories
and minimum resolvable deltas.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Generate attack token sequences")
    parser.add_argument("--output-dir", type=Path, default=ATTACK_DIR)
    parser.add_argument("--folds", nargs="+", type=int, default=FOLDS)
    args = parser.parse_args()

    setup_logging("generate_attacks")
    args.output_dir.mkdir(parents=True, exist_ok=True)

    vocab = load_vocab()

    # Load test targets for all folds
    logger.info("Loading test targets for all folds (from cached inference)...")
    all_targets = {}
    all_op_types = {}
    for fold in args.folds:
        cached = load_cached_targets(fold)
        target_tokens = cached["target_tokens"]
        all_targets[fold] = target_tokens
        all_op_types[fold] = cached["operation_type"].numpy()
        logger.info(f"  Fold {fold}: {target_tokens.shape[0]} windows, seq_len={target_tokens.shape[1]}")
    ...
    logger.info(f"All attacks generated in {elapsed:.1f}s")
