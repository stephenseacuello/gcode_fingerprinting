...
However, this headline number is misleading: V7's numeric accuracy is only 16.7\%, meaning it correctly predicts fewer than one in six numeric tokens.
V7 ``succeeds'' primarily by predicting special tokens (BOS, EOS, PAD) and parameter-type tokens while largely failing on the most informative numeric values.

V8 reverses this pattern, achieving 43.4\% numeric accuracy (2.6$\times$ V7) and 88.7\% command accuracy (+14.9~pp).
Multi-line context drives these gains: seeing preceding G-code lines (e.g., G1 X-10.0 Y5.0 followed by G1 X-10.5 Y5.0), the decoder infers subsequent coordinates from the sequential pattern rather than from sensor signals alone.
...
The best scoring method varies by attack type: V7 $S_{\text{mean}}$ for structural attacks (A1, A3), V7 $S_{\text{rank}}$ for small coordinate perturbations (A2, $\delta \leq 0.1$~mm), V8 $S_{\max}$ for large perturbations (A2, $\delta \geq 1.0$~mm), and V8 $S_{\text{mean}}$ for cross-operation attacks (A4).
This argues decisively against relying on any single anomaly score.
...
The V7 result falls below chance, indicating an inverse correlation: V7 assigns \emph{lower} NLL to cross-operation G-code than to the original, likely because foreign sequences are simpler and easier to predict regardless of sensor context.
V8 avoids this by using multi-line context, which flags cross-operation blocks as highly surprising.

\paragraph{Rank-based scoring outperforms NLL for coordinate attacks.}
On A2 with $\delta = 0.001$~mm, V7 Rank achieves AUROC = 0.859 versus V7 NLL = 0.697 (+16.2~pp; pooled DeLong test $p = 1.2\times10^{-5}$, Holm--Bonferroni-corrected).
This advantage holds and strengthens across magnitudes up to $\delta = 0.1$~mm (+20.7~pp at $\delta = 0.01$, Holm-adjusted $p = 5.1\times10^{-15}$; +17.1~pp at $\delta = 0.1$, Holm-adjusted $p = 1.1\times10^{-13}$). Rank-based scoring detects relative probability reordering regardless of absolute scale (Section~\ref{sec:rank_score}).
...
Figure~\ref{fig:roc_curves} shows ROC curves for four representative attacks, and Figure~\ref{fig:score_distributions} shows score distributions for normal versus attacked windows.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/roc_curves.pdf}
\caption{ROC curves for four attack types. (a)~A1 command swap: per-fold curves (thin) and mean (thick). (b)~A2 coordinate perturbation ($\delta = 0.1$~mm): NLL (blue) vs.\ rank scoring (orange), showing rank's clear advantage. (c)~A3 parameter swap. (d)~A4 cross-operation swap: V7 (blue) vs.\ V8 (red), showing V8's dramatic improvement from multi-line context.}
...
\paragraph{Counterintuitive perturbation-magnitude dependence.}
Larger perturbations are not always easier to detect.
Under V7, $S_{\text{rank}}$ AUROC drops from 0.859 ($\delta = 0.001$) to 0.599 ($\delta = 50$~mm) because large perturbations push the axis value into a regime where the decoder's distribution is nearly uniform---both original and perturbed values become equally implausible.
V8 partially recovers at large $\delta$ ($S_{\max}$ AUROC = 0.797) because multi-line context constrains the plausible range: a coordinate wildly inconsistent with surrounding lines produces a strong $S_{\max}$ signal.
...
Z-axis $S_{\text{mean}}$ AUROC is \emph{bit-identical} (0.542) across all ten magnitudes from 0.001 to 50~mm. This exact invariance is a tokenization artifact rather than a detectability plateau: Z values are sparse in the corpus (depth changes are infrequent), so most Z perturbations round back to the original token and produce no change at all. We therefore restrict coordinate-attack claims to the X and Y axes, where the vocabulary is dense enough to realize the requested perturbation.
...
This is not a deficiency of those methods but a structural fact of the threat model: an A1/A3 attack modifies only the \emph{claimed} G-code and leaves the physical process (and hence every sensor channel) unchanged, so a sensor-only detector receives an identical input for the normal and attacked windows and cannot separate them even in principle (Section~\ref{sec:disc_multiscale}).
...
This same property reverses for \emph{physical damage} detection, where the attack \emph{does} alter the sensor signal. Using the V8 pipeline, the encoder detects damage well---AUROC = 0.874 (damage vs.\ normal adaptive), 0.949 (face), and 0.703 (pocket)---while the decoder is weaker (0.465, 0.747, 0.416), because altered sensors can yield reconstructions that happen to remain consistent with the claimed G-code. (The shorter V7 pipeline gives weaker, mixed damage AUROCs for both.) Encoder and decoder thus address complementary threats: the encoder for physical-process anomalies that perturb the sensors, the decoder for G-code integrity violations that do not.
...
We partition numeric tokens by the decoder's per-token confidence---the maximum axis-conditional probability $\max_v p(v)$ on normal data (median 0.51)---and measure per-token detection AUROC under an adjacent-token (minimum-resolvable) perturbation:

\begin{itemize}
    \item \textbf{Confident tokens} ($\max_v p(v) > 0.5$, 54.3\% of numeric positions): the decoder has a clear preferred value, so a perturbation produces a large probability/rank shift. Per-token AUROC = \textbf{0.888}.
...
This is a continuum, not a clean bimodal split: the decoder's confidence is bounded away from zero (no numeric token has full-vocabulary $\max_v p(v) < 0.042$), so there is no genuinely degenerate ``uniform'' population. Detectability instead rises smoothly with per-token confidence, a consequence of V7's limited single-line context---without surrounding lines, many coordinates remain only partially constrained.

Two implications follow:

\begin{enumerate}
    \item \textbf{Aggregate AUROC is a confidence-weighted average.} The reported 0.697 (A2, $\delta = 0.001$, $S_{\text{mean}}$) pools highly detectable confident tokens with near-undetectable low-confidence ones. Rank-based scoring (Section~\ref{sec:rank_score}) recovers much of the lost signal precisely because it is invariant to absolute probability magnitude, lifting AUROC to 0.859 on the same attack.
...
\begin{table}[t]
\centering
\caption{Calibration results for the V7 decoder (mean across 5 folds). Temperature scaling \emph{degrades} ECE on the legacy head, the primary head used for anomaly scoring. The command head ($T = 1.0$) is already at its optimum. The legacy head's learned temperature $T = 1.287$ distorts the probability distribution.}
...
For V8, the effect reverses: legacy-head ECE \emph{increases} from 0.176 to 0.242 with the same $T$.
The command and parameter heads each predict among only 6 and 10 classes respectively, with highly skewed distributions (G1 accounts for $>$70\% of command tokens). High ECE in these heads reflects this extreme class imbalance rather than poor probability ordering. Since anomaly scoring uses NLL (log-probability of the specific target token) rather than calibrated confidence, the miscalibrated bin-level statistics do not affect detection performance.

Temperature scaling produces inconsistent effects: a marginal ECE improvement for V7 (0.353$\to$0.309) but degradation for V8 (0.176$\to$0.242).
Since anomaly scoring relies on NLL rank-ordering rather than calibrated probabilities, and the optimal temperature varies unpredictably across decoders, we use \emph{uncalibrated} logits for all scoring.
...
On the structural and semantic attacks reported in Table~\ref{tab:baselines} (A1, A3, A4), all four baselines sit near chance (AUROC 0.46--0.56).
This is expected: because the attacker modifies G-code without altering physical execution, the sensor signals are unchanged, so a sensor-only method has no genuine detection signal.
We caution against reading this as an exact 0.500 for \emph{every} attack: for A2 coordinate perturbations the \emph{unpaired} baselines scatter widely (e.g., at $\delta = 0.001$~mm, Coelho 1D-CNN reaches AUROC 0.846 while Isolation Forest falls to 0.128). These deviations are a \emph{window-selection artifact}---only a subset of windows is perturbable at a given $\delta$, and that subset has distinguishable sensor characteristics, so the unpaired baselines discriminate \emph{which windows are attackable} rather than \emph{whether} an attack occurred (the well-below-chance Isolation-Forest value confirms this is not detection).
...
This advantage comes at higher computational cost (27.2M parameters; measured decoder latency 5.8~ms/window on GPU, 22.3~ms on CPU, vs.\ $<$1~ms for statistical baselines), but the cost is justified since no simpler method can detect G-code integrity attacks at all.

For \emph{physical damage} detection (drive band removed), sensor-only baselines perform comparably: One-Class SVM achieves AUROC = $0.769 \pm 0.065$ and Coelho 1D-CNN achieves $0.729 \pm 0.130$, since damage directly alters sensor signals.
...
A2 X $\delta{=}0.001$ mm & 0.697 & 0.859 & \textbf{0.962} & $+0.103$ \\
A2 X $\delta{=}0.1$ mm & 0.676 & 0.847 & \textbf{0.866} & $+0.019$ \\
A2 X $\delta{=}1.0$ mm & 0.637 & 0.754 & \textbf{0.771} & $+0.017$ \\
...
On small coordinate perturbations (A2, $\delta = 0.001$~mm), it reaches AUROC = 0.962---a 10.3 percentage point improvement over rank-based scoring alone.
For cross-operation attacks (A4), it achieves 0.718 versus 0.623 for rank scoring (+9.5~pp).

The classifier's advantage comes from combining complementary signals: encoder memory captures sensor-level patterns, NLL captures per-token reconstruction quality, and rank scores capture within-axis ordering.
The only attack type where the classifier underperforms is A1 command swap (0.754 vs.\ NLL's 0.803), where the raw NLL spike from a single command substitution is already a strong signal that the classifier's additional features dilute.

Unlike the unsupervised scoring methods (NLL, rank, embedding distance), the XGBoost classifier requires labeled attack examples during training. It is therefore a \emph{supervised} detector that cannot generalize to unseen attack types without retraining. We include it to establish an upper bound on detection performance when attack signatures are known a priori.
...
We treat the unsupervised path as the deployable system (peak AUROC 0.803 on A1, 0.859 on small coordinate attacks) and the supervised 0.962 strictly as an upper bound on what is achievable when attack signatures are known a priori.

\subsection{Compound Attacks, Significance, and Adaptive Adversaries}
\label{sec:results_revision}

\paragraph{Compound attacks (A9).}
We evaluate a grammar-compliant compound attack that applies both a command swap (A1) and an $X$-coordinate shift ($\delta = 0.1$~mm) within the same window. Detection is \emph{stronger} than for either component alone: V7 $S_{\text{mean}}$ reaches AUROC = $0.872 \pm 0.039$ (versus 0.803 for A1 and 0.676 for A2 at $\delta = 0.1$ alone), because the two perturbations contribute independent surprise to the short token sequence.
...
The black-box results above assume a \emph{non-adaptive} attacker that applies a fixed perturbation. We additionally model a first-order adaptive attacker that, per window, selects the perturbation magnitude ($\delta \geq 0.1$~mm, i.e.\ still sabotage-relevant) minimizing the detector's score. Against the V7 $S_{\max}$ scorer, detection collapses from AUROC = 0.790 (fixed $\delta = 0.1$~mm) to \textbf{0.541} (near chance)---an evasion drop of 0.249. An attacker who can probe the detector can therefore largely evade single-score detection by choosing where on the magnitude--detectability curve to operate, and we accordingly frame all detection numbers as detection-under-non-adaptive-attack (Section~\ref{sec:limitations}).
