...
NLL-based scores measure the surprisal the decoder assigns to the claimed G-code sequence.
...
\begin{equation}
    ...
    \label{eq:s_max}
\end{equation}
For the V8 multi-line decoder, where most tokens in an attacked window may be unmodified, $S_{\max}$ avoids diluting a localized anomaly.
If an A2 perturbation modifies a single X value within a 63-token sequence, $S_{\text{mean}}$ averages the anomalous token with 62 normal tokens, while $S_{\max}$ captures it directly.
...
For V8 ($T \leq 63$), the two scores diverge, with $S_{\max}$ outperforming on localized attacks (e.g., single coordinate perturbation) and $S_{\text{mean}}$ outperforming on diffuse attacks (e.g., A4 cross-operation swap).
...
The decoder's \emph{ranking} may still favor the original value, however.
If the original has rank 45 and the perturbed token rank 120, the rank score detects the shift despite negligible absolute probabilities.
Empirically, rank-based scoring outperforms NLL by up to 16 percentage points.
On A2 with $\delta = 0.001$~in, V7 Rank achieves AUROC = 0.859 versus V7 NLL = 0.697 (Section~\ref{sec:results_anomaly}).
...
Multi-head disagreement achieves AUROC $= 0.500$, no better than chance, across all attack types and perturbation magnitudes in both decoders.
The jointly trained heads share the same Transformer trunk and produce highly correlated outputs.
Under attack, all heads are confused in the same direction, producing consistent but incorrect predictions.
The baseline disagreement rate is only 0.008, providing insufficient variance for discrimination.
We report this negative result because multi-head disagreement has been proposed as a general-purpose uncertainty signal.
Our results show it is ineffective when heads share a common backbone.
...
\begin{equation}
    S_{\text{hybrid}} = \begin{cases}
        ...
        S_{\text{rank}}^{(\text{V7})} & \text{for small coordinate perturbations (A2, $\delta \leq 0.1$~in)} \\
        S_{\max}^{(\text{V8})} & \text{for large coordinate perturbations (A2, $\delta > 0.1$~in)} \\
        S_{\text{mean}}^{(\text{V8})} & \text{for semantic/cross-operation attacks (A4)}
    \end{cases}
    \label{eq:s_hybrid}
\end{equation}
...
\begin{equation}
    S_{\text{hybrid-max}} = \max\left\{ \tilde{S}_{\text{mean}}^{(\text{V7})}, \; \tilde{S}_{\text{rank}}^{(\text{V7})}, \; \tilde{S}_{\max}^{(\text{V8})}, \; \tilde{S}_{\text{mean}}^{(\text{V8})} \right\}
    \label{eq:s_hybrid_max}
\end{equation}
where $\tilde{S}$ denotes z-score normalization.
This max-over-normalized-scores approach trades per-attack-type optimality for generality.
The maximum over multiple scores has a heavier right tail, which slightly elevates the false positive rate.
We did not separately evaluate hybrid-max.
The grid-searched ensemble (Section~\ref{sec:results_anomaly_head}) provides a more principled weight optimization over the same component scores.
